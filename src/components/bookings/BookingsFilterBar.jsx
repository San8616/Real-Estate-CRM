...
import { BOOKING_STATUSES, BOOKING_TYPES, SALES_AGENTS } from '../../data/mockBookings';
...
export default function BookingsFilterBar({
  ...
  const statusTabs = [
    { id: '', label: 'All Bookings', count: statusCounts.all ?? 0 },
  ...
  return (
    <div className="space-y-3">
      {/* Top Status Tabs / Filter Chips row matching reference UI */}
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-slate-200/80 bg-white p-3.5 shadow-xs lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="relative flex-1 min-w-[240px] max-w-md">
          ...
          <input
            ...
            placeholder="Search by client, unit, booking ID, agent..."
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative min-w-[145px] flex-1 sm:flex-none">
            <select
              ...
              className="w-full appearance-none rounded-xl border border-slate-200 bg-slate-50/70 py-2 pl-3 pr-8 text-xs font-medium text-slate-700 transition-all hover:bg-slate-50 focus:border-purple-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-100 cursor-pointer"
            >
          ...
          {/* 3. Assigned Agent Dropdown */}
          <div className="relative min-w-[155px] flex-1 sm:flex-none">
            <select
              value={agentFilter}
              onChange={(e) => onAgentChange(e.target.value)}
              aria-label="Filter by Assigned Broker"
              className="w-full appearance-none rounded-xl border border-slate-200 bg-slate-50/70 py-2 pl-3 pr-8 text-xs font-medium text-slate-700 transition-all hover:bg-slate-50 focus:border-purple-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-100 cursor-pointer"
            >
